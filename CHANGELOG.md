...
### Tests
- ✅ **L1 生命周期测试**（7 个）— Serper/搜狗 init 验证、无 Key 优雅跳过、智谱不受影响
- ✅ **E2E 验证测试**（10 个）— 真实实例创建、HTTP API 可达、死代码已清理、路由正确
- ✅ **E2E Flask 应用测试**（3 个）— `create_app()` 启动验证、服务状态检查、`/api/blog/generate` 端到端
- ✅ **全量回归** — 28 个 75.10 相关测试全部通过，零回归
